...
int main() {
  ...
  // construct the map
  Map map(map_waypoints_x, map_waypoints_y,
          map_waypoints_s, map_waypoints_dx,
          map_waypoints_dy);
  // Path planner setup with the map
  Planner planner(map);
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            // For each car, it is a vector of [ id, x, y, vx, vy, s, d]
          	auto sensor_fusion = j[1]["sensor_fusion"];

            vector<PeerCar> peer_cars;
            for (auto sensor : sensor_fusion) {
              ...
              double s = sensor[5];
              double d = sensor[6];
              peer_cars.push_back(PeerCar(id, x, y, vx, vy, s, d));
            }
            ...
            tuple<Path, Path> path = planner.plan(sdc, peer_cars,
          	...
          	vector<double> next_x_vals = get<0>(path);
          	vector<double> next_y_vals = get<1>(path);

            report(sdc, peer_cars, planner);
